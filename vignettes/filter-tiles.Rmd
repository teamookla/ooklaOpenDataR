...
Choosing coordinates from somewhere like Google maps is flexible and quick, but not necessarily reproducible or sustainable. When you don't have a boundary shapefile, though, it's a good alternative. I'll explain how to do this with Google maps, but any source where you can find coordinates would work.

Let's say you're interested in the fixed broadband performance around Marrakesh, Morrocco but don't have a shapefile of the boundary on hand. We can still approximate it!

Type in the place in the search bar: `Marrakesh, Morrocco`

```{r search, echo = FALSE, out.width="500px"}
knitr::include_graphics("marrakesh_search.png")
```

Once you have the area highlighted, you can start choosing coordinates. I'll start at the top to get the `ymax` coordinate. You want to pick a point that's at least includes the northernmost edge of your area.

```{r marrakesh_top, echo = FALSE, out.width="500px"}
knitr::include_graphics("marrakesh_top.png")
```

I usually round my coordinates for simplicity so I'll say `ymax = 31.72`.

Next I'll repeat the process and click on the rightmost point to get the `xmax` coordinate.

```{r marrakesh_right, echo = FALSE, out.width="500px"}
knitr::include_graphics("marrakesh_right.png")
```

Based on this point I'll say `xmax = -7.88`. Repeat this process again for the bottom (`ymin`) and leftmost (`xmin`) coordinates. 

Once you've gathered all the coordinates, you can create the bounding box with the following code:

```{r bbox}
bbox <- st_bbox(c(xmin = -8.13, xmax = -7.88, ymax =31.72, ymin = 31.55), crs = st_crs(4326)) # 4326 means that the coordinates are latitudes and longitudes
```
...
Then to check that the tiles are in the right place I'll use `mapview()` to make a quick interactive map with a basemap.
...
If you do have a shapefile (or other geographic data file, geoJSON for example), the process is much simpler. I'll use Minneapolis, MN as an example here.
...
Then I'll download the fixed performance tiles for Q2 2020 and filter them with the Minneapolis boundary. 
